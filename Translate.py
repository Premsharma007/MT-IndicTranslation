...
# -----------------------------
# Main pipeline with timer
# -----------------------------
def pipeline(file, src_lang_label, tgt_lang_label, progress=gr.Progress()):
    if file is None:
    ...
    return preview, out_path, total_time_str
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    time_label = gr.Label(label="Total Time Taken (HH:MM:SS)")

    translate_btn.click(fn=pipeline, inputs=[file_in, src_dd, tgt_dd], outputs=[output_text, file_out, time_label])

if __name__ == "__main__":
    print("Starting Gradio interface...")
    demo.launch(server_port=7860, share=False, inbrowser=False, show_error=True)
